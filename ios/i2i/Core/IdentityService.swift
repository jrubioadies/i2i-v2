...
final class IdentityService {
    ...
    func loadOrCreate() throws -> LocalIdentity {
        if let stored = repository.load() {
            ...
            return try migrateIfNeeded(stored)
        }
        return try create()
    }
    ...
    private func create() throws -> LocalIdentity {
        ...
        try KeyStore.saveEncryptionPrivateKey(encryptionPrivateKey, tag: KeyStore.encryptionPrivateKeyTag)
        ...
        let identity = LocalIdentity(
            ...
            encryptionPublicKey: encryptionPrivateKey.publicKey.rawRepresentation
        )
        try repository.save(identity)
        current = identity
        return identity
    ...
    private func migrateIfNeeded(_ stored: LocalIdentity) throws -> LocalIdentity {
        var identity = stored

        if identity.encryptionPublicKey == nil || !KeyStore.exists(tag: KeyStore.encryptionPrivateKeyTag) {
            let encryptionPrivateKey = Curve25519.KeyAgreement.PrivateKey()
            try KeyStore.saveEncryptionPrivateKey(encryptionPrivateKey, tag: KeyStore.encryptionPrivateKeyTag)
            identity.encryptionPublicKey = encryptionPrivateKey.publicKey.rawRepresentation
            try repository.save(identity)
        }
